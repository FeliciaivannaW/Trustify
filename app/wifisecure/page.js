...
import { Wifi, ArrowRight, ShieldAlert, ArrowLeft } from 'lucide-react';
...
export default function WiFiSecure() {
  const { t } = useLang();
  ...
  const handleAnalyze = async () => {
    ...
    const res = await analyzeText(input, 'wifi'); 
    if (res) addScan({ ...res, type: 'wifisecure', snippet: input.substring(0, 30) });
    setResult(res);
      ...
      <main style={{ paddingTop: 80, paddingBottom: 60, flex: 1, background: 'var(--background)' }}>
        <div style={{ maxWidth: 800, margin: '0 auto', padding: '0 24px' }}>
          ...
          <div style={{ textAlign: 'center', marginBottom: 40 }}>
            ...
            <h1 style={{ fontSize: 32, fontWeight: 800, marginBottom: 12 }}>WiFiSecure</h1>
            <p style={{ color: 'var(--text-secondary)', fontSize: 16, maxWidth: 500, margin: '0 auto' }}>
              Cek tingkat keamanan jaringan WiFi publik atau nama WiFi (SSID) sebelum Anda terhubung untuk mencegah pencurian data.
            </p>
          </div>

          <div className="glass-card" style={{ padding: 24, marginBottom: 32 }}>
            <div style={{ position: 'relative', marginBottom: 16 }}>
              <ShieldAlert size={20} color="var(--text-muted)" style={{ position: 'absolute', left: 16, top: '50%', transform: 'translateY(-50%)' }} />
              <input
                type="text"
                className="trustify-input"
                placeholder="Ketik nama WiFi (SSID) di sini..."
                value={input}
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 16 }}>
              ...
              <button 
                ...
                disabled={isAnalyzing || input.length < 3}
                style={{ opacity: (isAnalyzing || input.length < 3) ? 0.7 : 1 }}
              >
                Analisa Jaringan <ArrowRight size={18} />
              </button>
            </div>
          </div>
          ...
          <AnimatePresence mode="wait">
            {isAnalyzing && (
              <motion.div key="loading" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                <div className="glass-card"><LoadingAnalysis message="Menganalisa keamanan WiFi..." /></div>
              </motion.div>
            )}
