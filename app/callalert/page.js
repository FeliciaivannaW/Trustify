...
import { PhoneCall, ArrowRight, Search, ArrowLeft } from 'lucide-react';
...
export default function CallAlert() {
  ...
  const handleAnalyze = async () => {
    ...
    if (res) addScan({ ...res, type: 'callalert', snippet: input.substring(0, 30) });
    setResult(res);
      ...
      <main style={{ paddingTop: 80, paddingBottom: 60, flex: 1, background: 'var(--background)' }}>
        <div style={{ maxWidth: 800, margin: '0 auto', padding: '0 24px' }}>
          ...
          <div style={{ textAlign: 'center', marginBottom: 40 }}>
            ...
            <h1 style={{ fontSize: 32, fontWeight: 800, marginBottom: 12 }}>CallAlert</h1>
            <p style={{ color: 'var(--text-secondary)', fontSize: 16, maxWidth: 500, margin: '0 auto' }}>
              Menerima telepon dari nomor tidak dikenal yang mengaku dari bank atau kepolisian? Cek riwayat laporannya di sini.
            </p>
          </div>
          ...
          <div className="glass-card" style={{ padding: 24, marginBottom: 32 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 16 }}>
              <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', color: 'var(--primary)', fontSize: 13, fontWeight: 600 }}>
                <span style={{ fontSize: 16 }}>📷</span> Upload Screenshot Log Panggilan
              ...
              <button className="btn-primary" onClick={handleAnalyze} disabled={isAnalyzing || input.length < 5}>
                Lacak Nomor <ArrowRight size={18} />
              </button>
            </div>
          </div>
          ...
          <AnimatePresence mode="wait">
            {isAnalyzing && (
              <motion.div key="loading" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                <div className="glass-card"><LoadingAnalysis message="Mencocokkan dengan database penipu..." /></div>
              </motion.div>
            )}
            {!isAnalyzing && result && (
