...
import { Briefcase, ArrowRight, FileText, ArrowLeft } from 'lucide-react';
...
export default function LowonganAman() {
  ...
  const handleAnalyze = async () => {
    ...
    const res = await analyzeText(input, 'job');
    if (res) addScan({ ...res, type: 'lowonganaman', snippet: input.substring(0, 30) });
    setResult(res);
      ...
      <main style={{ paddingTop: 80, paddingBottom: 60, flex: 1, background: 'var(--background)' }}>
        <div style={{ maxWidth: 800, margin: '0 auto', padding: '0 24px' }}>
          ...
          <div style={{ textAlign: 'center', marginBottom: 40 }}>
            ...
            <h1 style={{ fontSize: 32, fontWeight: 800, marginBottom: 12 }}>LowonganAman</h1>
            <p style={{ color: 'var(--text-secondary)', fontSize: 16, maxWidth: 500, margin: '0 auto' }}>
              Tawaran gaji puluhan juta tanpa syarat? Cek deskripsi lowongan kerja sebelum mengirimkan data pribadi atau mentransfer uang "admin".
            </p>
          </div>
          ...
          <div className="glass-card" style={{ padding: 24, marginBottom: 32 }}>
            <div style={{ position: 'relative', marginBottom: 16 }}>
              ...
              <textarea
                ...
                placeholder="Tempel teks deskripsi lowongan kerja (loker) di sini..."
                value={input}
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 16 }}>
              <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', color: 'var(--primary)', fontSize: 13, fontWeight: 600 }}>
                <span style={{ fontSize: 16 }}>📷</span> Upload Banner/Screenshot
              ...
              <button className="btn-primary" onClick={handleAnalyze} disabled={isAnalyzing || input.length < 10}>
                Analisa Lowongan <ArrowRight size={18} />
              </button>
            </div>
          </div>
          ...
          <AnimatePresence mode="wait">
            {isAnalyzing && (
              <motion.div key="loading" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                <div className="glass-card"><LoadingAnalysis message="Mencocokkan pola bahasa rekrutmen palsu..." /></div>
              </motion.div>
            )}
            {!isAnalyzing && result && (
